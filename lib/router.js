 ...
 * - postExec: Trigger immediately after a chain is kicked off. This may fire before the first command is complete.
...
/*
  ...
  queue.on('execNext', function (cxt) {queue.execNext(cxt);})

  // We want these events to be bubbled.
  this.forwardEvents(queue, ['commandContinue', 'commandComplete', 'beforeCommand', 'beforeCommandInit', 'error', 'commandListComplete']);
...
Router.prototype.doCommand = function(requestName, spec, context, index) {
  ...
  if (index >= spec.length) {
    ...
    this.emit('commandListInterrupted', context);
    return;
  }
