...
Router.prototype.handleRequest = function(request, cxt, tainted) {
  ...
  if (!!tainted && request.length > 0 && request[0] == '@') {
    throw new Error('A tainted request cannot call an internal @-route.');
  }
...
Router.prototype.doCommand = function(requestName, spec, context, index) {
  ...
  cmd.on('error', function (err) {
    // Experimental: We need to notify the main chain that we have been
    // interrupted. The error handling code may execute a second route
    // against the same request, though. Not sure if under all
    // circumstances this will work. It is possible that a slow error
    // response (one that takes a few ticks) may have the request
    // closed from under it.
    router.emit('commandListInterrupted', requestName, spec, context);

    // This prevents the error route from running:
    //router.stop = true;
